...
(defun x-face-mule-load-cache-file ()
  "Load X-Face cache file.  Return an alist if succeeded."
  (setq x-face-mule-x-face-to-rectangle-cache nil)
  (when (and x-face-mule-use-cache-file
    ...
    (with-temp-buffer
      ...
      (when (string-match "^21\\.\\(?:0\\.\\|1\\.\\(?:[[:digit:]]+$\\|\
\\(?:[[:digit:]]\\|[0-4][[:digit:]]\\|50\\)\\.[[:digit:]]+$\\)\\)"
			  emacs-version)
	;; Just one more of the mysteries of this world that we can get
	;; the correct data by re-reading a file in the same buffer
	;; even if Emacs 21.1 has a bug in coding.c.
	(erase-buffer)
	(insert-file-contents-as-coding-system
	 x-face-mule-image-file-coding-system x-face-mule-cache-file))
	...
	(let ((start (point))
	      ...
	      ;; Mule 2 would fail to search images when `mc-flag' is
	      ;; non-nil.
	      mc-flag)
	  (while (and (setq end (condition-case nil
				    (forward-list 1)
				  (error nil)))
		      (progn
			(goto-char start)
			(search-forward "(" end t))
		      (setq x-face (condition-case nil
				       (read (current-buffer))
				     (error nil))))
	    (set-text-properties 0 (length x-face) nil x-face)
	    (setq x-face (list x-face))
	    (while (re-search-forward "\"\\([^\"]+\\)\"" end t)
	      (push (match-string 1) x-face))
	    (when (= 4 (length x-face))
	      (push (nreverse x-face) x-face-mule-x-face-to-rectangle-cache))
	    (goto-char (setq start end))))
	(setq x-face-mule-x-face-to-rectangle-cache
	      (nreverse x-face-mule-x-face-to-rectangle-cache))))))
...
(defun x-face-mule-view-cache ()
  "View cached X-Face images."
  (interactive)
  (if (or x-face-mule-x-face-to-rectangle-cache
	  (x-face-mule-load-cache-file))
      (let ((cache x-face-mule-x-face-to-rectangle-cache)
	    (buffer (get-buffer-create "*cached X-Face images*"))
	    width xmax yindex xindex)
	(switch-to-buffer buffer)
	(erase-buffer)
	(set-buffer-multibyte t)
	(delete-other-windows)
	(setq width (/ (window-width) 8))
	(while cache
	  (setq xmax (min width (length cache))
		yindex 1)
	  (while (< yindex 4)
	    (setq xindex 0)
	    (while (< xindex xmax)
	      (put-text-property (point)
				 (progn
				   (insert (nth yindex (nth xindex cache)))
				   (point))
				 'face 'x-face-mule-highlight-x-face-face)
	      (insert "  ")
	      (setq xindex (1+ xindex)))
	    (backward-delete-char 2)
	    (insert "\n")
	    (setq yindex (1+ yindex)))
	  (setq cache (nthcdr xmax cache))
	  (when cache
	    (insert "\n")))
	(set-buffer-modified-p nil))
    (message "No images in the cache")))
